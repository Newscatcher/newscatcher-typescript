// This file was auto-generated by Fern from our API Definition.

import * as NewscatcherApi from "../../src/api/index";
import { NewscatcherApiClient } from "../../src/Client";
import { mockServerPool } from "../mock-server/MockServerPool";

describe("LatestHeadlinesClient", () => {
    test("latestHeadlinesGet (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = {
            status: "status",
            total_hits: 1,
            page: 1,
            total_pages: 1,
            page_size: 1,
            articles: [
                {
                    title: "title",
                    author: "author",
                    authors: ["authors"],
                    journalists: ["journalists"],
                    published_date: "published_date",
                    published_date_precision: "published_date_precision",
                    updated_date: "updated_date",
                    updated_date_precision: "updated_date_precision",
                    parse_date: "parse_date",
                    link: "link",
                    domain_url: "domain_url",
                    full_domain_url: "full_domain_url",
                    name_source: "name_source",
                    is_headline: true,
                    paid_content: true,
                    parent_url: "parent_url",
                    country: "country",
                    rights: "rights",
                    rank: 1,
                    media: "media",
                    language: "language",
                    description: "description",
                    content: "content",
                    title_translated_en: "title_translated_en",
                    content_translated_en: "content_translated_en",
                    word_count: 1,
                    is_opinion: true,
                    twitter_account: "twitter_account",
                    all_links: ["all_links"],
                    all_domain_links: ["all_domain_links"],
                    all_links_data: [
                        {
                            domain_url: "amazon.de",
                            link: "https://www.amazon.de/s?k=Künstliche+Intelligenz",
                            text: "KI Brillen",
                        },
                    ],
                    id: "id",
                    score: 1.1,
                    robots_compliant: true,
                    custom_tags: { key: ["value"] },
                    additional_domain_info: {
                        is_news_domain: true,
                        news_type: "News and Blogs",
                        news_domain_type: "Original Content",
                    },
                },
            ],
            user_input: { key: "value" },
        };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.latestHeadlines.latestHeadlinesGet({
            when: "7d",
            by_parse_date: true,
            lang: "en,es",
            not_lang: "fr,de",
            countries: "US,CA",
            not_countries: "UK,FR",
            predefined_sources: "top 50 US, top 20 GB",
            sources: "nytimes.com,finance.yahoo.com",
            not_sources: "cnn.com,wsj.com",
            not_author_name: "John Doe, Jane Doe",
            ranked_only: true,
            is_headline: true,
            is_opinion: true,
            is_paid_content: false,
            parent_url: "wsj.com/politics,wsj.com/tech",
            all_links: "https://aiindex.stanford.edu/report,https://www.stateof.ai",
            all_domain_links: "who.int,nih.gov",
            all_links_text: "Nvidia,Tesla",
            word_count_min: 300,
            word_count_max: 1000,
            page: 2,
            page_size: 50,
            clustering_enabled: true,
            clustering_threshold: 0.6,
            include_translation_fields: true,
            include_nlp_data: true,
            has_nlp: true,
            theme: "Finance,Tech",
            not_theme: "Crime,Sports",
            ORG_entity_name: '"Apple Inc" OR Microsoft',
            PER_entity_name: '"Elon Musk" OR "Jeff Bezos"',
            LOC_entity_name: '"San Francisco" OR "New York City"',
            MISC_entity_name: 'AWS OR "Microsoft Azure"',
            title_sentiment_min: -0.5,
            title_sentiment_max: 0.5,
            content_sentiment_min: -0.5,
            content_sentiment_max: 0.5,
            iptc_tags: "20000199,20000209",
            not_iptc_tags: "20000205,20000209",
            iab_tags: "Business,Events",
            not_iab_tags: "Agriculture,Metals",
            custom_tags: "Tag1,Tag2",
            robots_compliant: true,
        });
        expect(response).toEqual({
            status: "status",
            total_hits: 1,
            page: 1,
            total_pages: 1,
            page_size: 1,
            articles: [
                {
                    title: "title",
                    author: "author",
                    authors: ["authors"],
                    journalists: ["journalists"],
                    published_date: "published_date",
                    published_date_precision: "published_date_precision",
                    updated_date: "updated_date",
                    updated_date_precision: "updated_date_precision",
                    parse_date: "parse_date",
                    link: "link",
                    domain_url: "domain_url",
                    full_domain_url: "full_domain_url",
                    name_source: "name_source",
                    is_headline: true,
                    paid_content: true,
                    parent_url: "parent_url",
                    country: "country",
                    rights: "rights",
                    rank: 1,
                    media: "media",
                    language: "language",
                    description: "description",
                    content: "content",
                    title_translated_en: "title_translated_en",
                    content_translated_en: "content_translated_en",
                    word_count: 1,
                    is_opinion: true,
                    twitter_account: "twitter_account",
                    all_links: ["all_links"],
                    all_domain_links: ["all_domain_links"],
                    all_links_data: [
                        {
                            domain_url: "amazon.de",
                            link: "https://www.amazon.de/s?k=K\u00FCnstliche+Intelligenz",
                            text: "KI Brillen",
                        },
                    ],
                    id: "id",
                    score: 1.1,
                    robots_compliant: true,
                    custom_tags: {
                        key: ["value"],
                    },
                    additional_domain_info: {
                        is_news_domain: true,
                        news_type: "News and Blogs",
                        news_domain_type: "Original Content",
                    },
                },
            ],
            user_input: {
                key: "value",
            },
        });
    });

    test("latestHeadlinesGet (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.BadRequestError);
    });

    test("latestHeadlinesGet (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.UnauthorizedError);
    });

    test("latestHeadlinesGet (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.ForbiddenError);
    });

    test("latestHeadlinesGet (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(408)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.RequestTimeoutError);
    });

    test("latestHeadlinesGet (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(422)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.UnprocessableEntityError);
    });

    test("latestHeadlinesGet (7)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.TooManyRequestsError);
    });

    test("latestHeadlinesGet (8)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });

        const rawResponseBody = "string";

        server
            .mockEndpoint()
            .get("/api/latest_headlines")
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesGet();
        }).rejects.toThrow(NewscatcherApi.InternalServerError);
    });

    test("latestHeadlinesPost (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = { when: "7d", page_size: 1 };
        const rawResponseBody = {
            status: "status",
            total_hits: 1,
            page: 1,
            total_pages: 1,
            page_size: 1,
            articles: [
                {
                    title: "title",
                    author: "author",
                    authors: ["authors"],
                    journalists: ["journalists"],
                    published_date: "published_date",
                    published_date_precision: "published_date_precision",
                    updated_date: "updated_date",
                    updated_date_precision: "updated_date_precision",
                    parse_date: "parse_date",
                    link: "link",
                    domain_url: "domain_url",
                    full_domain_url: "full_domain_url",
                    name_source: "name_source",
                    is_headline: true,
                    paid_content: true,
                    parent_url: "parent_url",
                    country: "country",
                    rights: "rights",
                    rank: 1,
                    media: "media",
                    language: "language",
                    description: "description",
                    content: "content",
                    title_translated_en: "title_translated_en",
                    content_translated_en: "content_translated_en",
                    word_count: 1,
                    is_opinion: true,
                    twitter_account: "twitter_account",
                    all_links: ["all_links"],
                    all_domain_links: ["all_domain_links"],
                    all_links_data: [
                        {
                            domain_url: "amazon.de",
                            link: "https://www.amazon.de/s?k=Künstliche+Intelligenz",
                            text: "KI Brillen",
                        },
                    ],
                    id: "id",
                    score: 1.1,
                    robots_compliant: true,
                    custom_tags: { key: ["value"] },
                    additional_domain_info: {
                        is_news_domain: true,
                        news_type: "News and Blogs",
                        news_domain_type: "Original Content",
                    },
                },
            ],
            user_input: { key: "value" },
        };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.latestHeadlines.latestHeadlinesPost({
            when: "7d",
            page_size: 1,
        });
        expect(response).toEqual({
            status: "status",
            total_hits: 1,
            page: 1,
            total_pages: 1,
            page_size: 1,
            articles: [
                {
                    title: "title",
                    author: "author",
                    authors: ["authors"],
                    journalists: ["journalists"],
                    published_date: "published_date",
                    published_date_precision: "published_date_precision",
                    updated_date: "updated_date",
                    updated_date_precision: "updated_date_precision",
                    parse_date: "parse_date",
                    link: "link",
                    domain_url: "domain_url",
                    full_domain_url: "full_domain_url",
                    name_source: "name_source",
                    is_headline: true,
                    paid_content: true,
                    parent_url: "parent_url",
                    country: "country",
                    rights: "rights",
                    rank: 1,
                    media: "media",
                    language: "language",
                    description: "description",
                    content: "content",
                    title_translated_en: "title_translated_en",
                    content_translated_en: "content_translated_en",
                    word_count: 1,
                    is_opinion: true,
                    twitter_account: "twitter_account",
                    all_links: ["all_links"],
                    all_domain_links: ["all_domain_links"],
                    all_links_data: [
                        {
                            domain_url: "amazon.de",
                            link: "https://www.amazon.de/s?k=K\u00FCnstliche+Intelligenz",
                            text: "KI Brillen",
                        },
                    ],
                    id: "id",
                    score: 1.1,
                    robots_compliant: true,
                    custom_tags: {
                        key: ["value"],
                    },
                    additional_domain_info: {
                        is_news_domain: true,
                        news_type: "News and Blogs",
                        news_domain_type: "Original Content",
                    },
                },
            ],
            user_input: {
                key: "value",
            },
        });
    });

    test("latestHeadlinesPost (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.BadRequestError);
    });

    test("latestHeadlinesPost (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(401)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.UnauthorizedError);
    });

    test("latestHeadlinesPost (4)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(403)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.ForbiddenError);
    });

    test("latestHeadlinesPost (5)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(408)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.RequestTimeoutError);
    });

    test("latestHeadlinesPost (6)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(422)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.UnprocessableEntityError);
    });

    test("latestHeadlinesPost (7)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = { message: "message", status_code: 1, status: "status" };

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(429)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.TooManyRequestsError);
    });

    test("latestHeadlinesPost (8)", async () => {
        const server = mockServerPool.createServer();
        const client = new NewscatcherApiClient({ maxRetries: 0, apiKey: "test", environment: server.baseUrl });
        const rawRequestBody = {};
        const rawResponseBody = "string";

        server
            .mockEndpoint()
            .post("/api/latest_headlines")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(500)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.latestHeadlines.latestHeadlinesPost();
        }).rejects.toThrow(NewscatcherApi.InternalServerError);
    });
});
