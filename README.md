...
const request: NewscatcherApi.GetSearchRequest = {
    ...
};
```
...
```typescript
import { SearchClient } from 'newscatcher-sdk/search';
...
```typescript
const { data, rawResponse } = await client.search.post(...).withRawResponse();
